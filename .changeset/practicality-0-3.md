...
- `the_board_api_get` の応答を `{data, pagination, truncated}` の JSON に変更 (破壊的)。既定 `format: concise` は compact JSON で null キーを省略 (空配列・0・false・"" は保持)。`format: detailed` で従来に近い整形 JSON。`fields` で返すキーを指定可能。上限 (`THE_BOARD_MAX_RESPONSE_CHARS`、既定 20,000 字) 超過時はレコード単位で切り詰め `truncated: true`。
